...
def reporting ():
    '''
    Prints the current level for each resource when entering 'report' in the main menu
    '''
...
def check_resources(user_option,current_resources):
    '''
    Checks if the current resources are enough to make the selected beverage, if it's enough it will return a True statement, if not it will return the missing resource
    '''
...
def process_coins(user_option):
    '''
    It will ask the user how many of each coin will be inserted, will sum all of them, subtract the coffee value and refound the correspondent change. Finally if there's
    not enough for the selected drink, it will prompt the user a message letting him know that it was not possible to continue and the money will be refounded. 
    '''
    deposited_coins = []
    sum_coins = 0
    ...
    if sum_coins < MENU[user_option]['cost']:
        print(f"Sorry, that's not enough money, {user_option} costs ${MENU[user_option]['cost']} and you have put ${round(sum_coins,2)}, your money have been refounded.")
        return False
    else:
...
def update_resources(user_option):
    '''
    Once there's enough resources and money to make the coffee, the resources will be subtracted, according to the amount specified in the MENU variable.
    '''
...
def make_coffee(user_option):
    '''
    Checks all requirements to make a coffee (money and resources), if all of them are True then it will give out the coffee, if not 
    a message will be prompt to the user letting him know that there are not enough resources
    '''
...
def check_user_input(user_option):
    '''
    This function works as a menu for all cases that user_input would be, in case it's not any of the listed then it will provide a message saying that it's not a valid option
    '''
    match user_option:
